...
using Plots
# change the location of the default legend top right and its default size
default(; legend=:outertopright, size=(700,400))

using Random
# setting the seed fr the reproducibility of this notebook
Random.seed!(42)

# load toy regression dataset taken from GPs examples.
x = [
    0.8658165855998895,
    0.6661700880180962,
    0.8049218148148531,
...
# we instantiate a Gaussia process with a Matern kernel. 
# The kernel has fixed variance and length 
# scale parameters of default value 1
f = GP(Matern52Kernel())
# GP{
    # AbstractGPs.ZeroMean{Float64},
    # Matern52Kernel{Distances.Euclidean}
    # }(
    #     AbstractGPs.ZeroMean{Float64}(),
    #     Matern 5/2 Kernel (metric = Distances.Euclidean(0.0))
    #     )
# we compute the projections of inputs of the training dataset observed under Gaussian noise
# with variance = 0.1
fx = f(x_train, 0.1) # ?prior

# we compute the log-likelihood of the outputs of the training dataset
logpdf(fx, y_train) # ?log-likelihood

# we compute the posterior gaussian process given the training data and calculate the
# log-likelihood of the test dataset
p_fx = posterior(fx, y_train) # posterior
logpdf(p_fx(x_test), y_test)
# -232.51565975767468

# we plot the posterior gaussian process given the training data, 
# and calculate the log-likelihood of the test dataset
# we plot the posterior gaussian process
# its mean and a ribbon of 2 standard deviations around its
# on a grid along with the observations
scatter(
    x_train,
    y_train;
    xlim=(0, 1),
...
#= 
previously we computed the log likelihood
of the untuned kernel parameters of the GP.
We now also perform approximate inference over said kernel parameters
using different Markov Chain Monte Carlo (MCMC) methods.
I.e. we approximate the posterior Distributions
of the kernel parameters with samples from a Markon Chain.
=#
...
# we define a Gaussian prior for the joint distribution of the two
# transformed kernel parameters. we assume that both parameters are indipendet with mean 0 and variance 1.
...
# kernel parameters. these samples are in the uncostrained space R^2.

samples, _ = sample(hamiltonian, proposal, initial_params, n_samples, adaptor, n_adapts; progress=false)
...
│    StanHMCAdaptor(
│        pc=WelfordVar,
│        ssa=NesterovDualAveraging(γ=0.05, t_0=10.0, κ=0.75, δ=0.8, state.ϵ=0.7860634926521906),
│        init_buffer=75, term_buffer=50, window_size=25,
│        state=window(76, 950), window_splits(100, 150, 250, 450, 950)
│    )
│   κ.τ.integrator = Leapfrog(ϵ=0.786)
└   h.metric = DiagEuclideanMetric([1.2182590655557473, 0.9004 ...])
┌ Info: Finished 2000 sampling steps for 1 chains in 1.210901019 (s)
│   h = Hamiltonian(metric=DiagEuclideanMetric([1.2182590655557473, 0.9004 ...]))
│   κ = HMCKernel{AdvancedHMC.FullMomentumRefreshment, Trajectory{MultinomialTS, Leapfrog{Float64}, GeneralisedNoUTurn{Float64}}}(AdvancedHMC.FullMomentumRefreshment(), Trajectory{MultinomialTS}(integrator=Leapfrog(ϵ=0.786), tc=GeneralisedNoUTurn{Float64}(10, 1000.0)))
│   EBFMI_est = 1.109796977454056
└   average_acceptance_rate = 0.8581266937466369
=#
...
# The vertical line in each graph indicates the mean
# of the samples.
histogram(
...
# Giulia's comment: they remember me a Gamma's distribution. you too?

#=
We approximate the log-likelihood of the test data 
using the posterior Gaussian processes for kernels with 
the sampled kernel parameters. We can observe that there 
is a significant improvement over the log-likelihood of 
the test data with respect to the posterior Gaussian process
with default kernel parameters of value 1.
=#
...
# We sample 5 functions from each posterior GP given by the final 
# 100 samples of kernel parameters.
# 
